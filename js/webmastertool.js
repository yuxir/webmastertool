	  ...
	  if(settings && settings.webmastertool) {
            ...
            if(settings.webmastertool['cloudflare_api_key']) {
              ...
              if(settings.webmastertool['show_cloudflare_tab']=='yes'){
                  ...
                  update_cloudflare_domains_info(settings.webmastertool['cloudflare_auth_email'], settings.webmastertool['cloudflare_api_key'], 'cloudflare-domains','cloudflare-dns');
    ...
    if($('input#vultr_api_key').val() || $('input#do_api_key').val() || $('input#linode_api_key').val() || $('input#heroku_api_key').val() || $('input#cloudflare_api_key').val()) {
        ...
        if($('input#cloudflare_api_key').val()) {
            ...
            if(($('#show_cloudflare_box:checked').length > 0)) {  
                ...
                update_cloudflare_domains_info($('input#cloudflare_auth_email').val(), $('input#cloudflare_api_key').val(), 'cloudflare-domains', 'cloudflare-dns');
            }else{
                // hide cloudflare tab
                $("div#tabs ul li:eq(5)").css("display", "none");
            }    
